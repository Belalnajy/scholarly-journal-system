import { ArrowRight, Save, UserPlus, Upload, X } from 'lucide-react';
...
import { UserRole, AcademicDegree } from '../../../types/user.types';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await activityLogsService.logUserAction(
        ...
        `تم إضافة مستخدم جديد: ${formData.name}`,
        { 
          created_user_id: newUser.id,
          created_user_name: formData.name,
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <DashboardHeader 
            ...
            subtitle="إنشاء حساب جديد للباحثين أو المحكمين أو المحررين"
          />
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                اختر الدور المناسب للمستخدم في النظام
              </p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6">
          ...
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2">
                الانتماء المؤسسي <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={formData.affiliation}
                onChange={(e) => setFormData({ ...formData, affiliation: e.target.value })}
                placeholder="جامعة الملك سعود - قسم علوم الحاسب"
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg focus:ring-2 focus:ring-[#0D3B66] focus:border-[#0D3B66] transition-all"
                required
              />
            </div>
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <span>{loading ? 'جاري الإضافة...' : 'إضافة المستخدم'}</span>
          </button>
          <button
            type="button"
            onClick={() => navigate('/dashboard/manage-users')}
