            ...
            <li key={item.id}>
              <Link
                ...
                className={`
                  ...
                  ${
                    isActive(item.path)
                      ? 'bg-[#F4D35E] text-[#0D3B66] font-semibold shadow-md'
                      : 'text-white hover:bg-white/10'
                  }
                `}
              >
                <span className={isActive(item.path) ? 'text-[#0D3B66]' : 'text-white'}>
                  {getIcon(item.icon)}
                </span>
        ...
        {userInfo && (
          <div className="flex items-center gap-3 mb-3 px-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-white/70 truncate">{userInfo.role}</p>
            </div>
          </div>
        )}
