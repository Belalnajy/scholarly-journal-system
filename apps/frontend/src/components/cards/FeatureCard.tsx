...
const getRouteForFeature = (title: string): string => {
  if (title.includes('تقديم البحث') || title.includes('تقديم بحث')) {
    return '/submit-research';
  } else if (title.includes('الأعداد')) {
    return '/issues';
  } else if (title.includes('هيئة التحرير')) {
    return '/editorial';
  } else if (title.includes('قواعد النشر')) {
    return '/publishing-rules';
  }
  return '#';
};
...
export function FeatureCard({ feature }: FeatureCardProps) {
  ...
  return (
    <motion.div 
      ...
      className={`flex h-auto min-h-[200px] w-full flex-col items-center justify-center gap-6 rounded-2xl bg-white px-4 py-6 shadow-[0px_0px_4px_0px_rgba(0,0,0,0.25)] transition-all hover:shadow-lg sm:min-h-[220px] sm:gap-7 sm:px-5 sm:py-7 lg:h-[250px] lg:w-[304px] lg:gap-8 lg:px-6 lg:py-8 ${route !== '#' ? 'cursor-pointer' : ''}`}
    >
      {/* Icon */}
